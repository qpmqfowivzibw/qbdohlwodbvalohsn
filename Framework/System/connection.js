...
const logMessage = async (msg, conn) => {
  ...
  if (isGroup) {
    // ✅ Group logic
    senderJid = msg.key.participantPn || "unknown@s.whatsapp.net";

    // If no participantPn (probably bot itself), set as bot JID
    if (!msg.key.participantPn) {
      senderJid = conn.user.id;
    }
  } else {
    // ✅ Private chat logic
    senderJid = msg.key.remoteJid || msg.sender || "unknown@s.whatsapp.net";
  }
