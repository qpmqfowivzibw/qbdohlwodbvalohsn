...
const saveChat = async (chat) => {
  ...
  try {
    // Use upsert - it handles both insert and update in a single atomic operation
    const [chatRecord, created] = await chatDb.upsert({
      id: chat.id,
      conversationTimestamp: chat.conversationTimestamp,
      isGroup,
    }, {
      returning: true // Ensure we get the record back
    });
...
const getName = async (jid) => {
  try {
    if (!jid || typeof jid !== "string") {
      console.warn("⚠️ Invalid JID passed to getName:", jid);
      return "Unknown";
    }
